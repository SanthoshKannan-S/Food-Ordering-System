...
const Cartall = mongoose.model('cartall', cartSchemaall);
...
router.post("/thankyou",async function(req,res){
  ...
  if(dosai <= dosai2 && idly <= idly2 && poori <= poori2 && pongal <= pongal2 && biriyani <= biriyani2 && rice<=rice2){
    ...
    if(check === false)
    {
      ...
      cartitem.cost=cost;
      await cartitem.save();
      const cartall = new Cartall({ username,total,dosai,idly,poori,pongal,biriyani,rice,cost }); 
      await cartall.save();
    }
    else if(check === true){
    ...
    var downloading = req.body.makepdf;
